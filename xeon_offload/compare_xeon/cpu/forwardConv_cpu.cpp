...
printf("\n\n");
  ...
  cv->output = (float *)malloc(sizeof(float) * totalOutputSize);
  if(cv->output == NULL) {
    printf(" Canont alloc memory \n");
    exit(-1);
  }
...
int forward_convolution(Convolution *c, int inputOffset, int outputOffset)
{
...
void Offload_work(Convolution *cv)
{
  int i, j;
